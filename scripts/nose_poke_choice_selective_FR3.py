...
USER_SOFTWARE_CONFIG_PATH = '/home/pi/makenzie_operant/setup_files/nose_poke_selective.yaml'
...
def run():
    ...
    FR2 = box.get_software_setting(location = 'values', setting_name='FR2', default = 2)
    tone = box.get_software_setting(location = 'checks', setting_name='tones_active', default = 1)
    choice_timeout = box.get_software_setting(location = 'values', setting_name='choice_timeout', default = 20)
    ...
    while total_time_phase.active():
        ...
        if poke_d1.pokes_reached and not d1_part_two:
            pokes_active_phase.end_phase()

            d1_part_two = True

            d1_reward_pokes_phase = box.timing.new_phase('d1_reward_pokes', length = choice_timeout)
            ...
            while d1_reward_pokes_phase.active():
        ...
        if poke_d2.pokes_reached and not d2_part_two:
            pokes_active_phase.end_phase()
            d2_part_two = True
            d2_reward_pokes_phase = box.timing.new_phase('d2_reward_pokes', length = choice_timeout)
            ...
            while d2_reward_pokes_phase.active():
